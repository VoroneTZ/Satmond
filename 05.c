...
{	
	...
	//Texture Movement
	you = ent_create("armor_knight.mdl", vector(-200,0,60), NULL);
	vec_scale(you.scale_x, 0.5);
	you.material = mtlTexMove;
	set(you, SHADOW);
	
	//Living Bloom Thingy
	you = ent_create("armor_knight.mdl", vector(200,-300,60), NULL);
	vec_scale(you.scale_x, 0.5);
	...
	//Dissolve
	ENTITY* dissolveObject = ent_create("armor_knight.MDL", vector(0,200,60), NULL);
	...
	while(1)
	{
		...
		dissolveObject.skill1 -= time_step;
		if(dissolveObject.skill1 <= 0) dissolveObject.skill1 = 100;
