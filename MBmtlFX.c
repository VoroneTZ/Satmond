...
function env_water_init()
{
	...
	if (bmap_waterbump)
		{
			ptr_remove(bmap_waterbump);							// *** correct mirror shift on multiple level loads
			bmap_waterbump = NULL;
		}
...
//id: 25
	...
	view_mirror.clip_near = 1;		// *** better close mirror
		...
		if (!is(my,CLIPPED)) 
			{
				...
				vec_for_max(view_mirror.portal_x,my);				// without taking into account scale
//				vec_mul(view_mirror.portal_x, my.scale_x);		// *** multiply with scale
				vec_add(view_mirror.portal_x,my.x);					// offset by position
//				view_mirror.portal_z = my.z;							// ***to avoid errors when vertex movements used
				mtlfx_mirrorvisible = 1;
			}
		wait(1);
	}
...
function mirror_water_init()
{
	if (bmap_water_uv)
		{
			ptr_remove(bmap_water_uv);							// *** correct mirror shift on multiple level loads
			bmap_water_uv = NULL;
		}
	
//	if (!bmap_water_uv) 
		{
//	   	bmap_water_uv = bmap_to_uv(bmap_create("water.dds"));					// orig - results in mirror offset
//	   	bmap_water_uv = bmap_to_normals(bmap_create("water.dds"), 50);		// *** - no mirror offset error !
   		bmap_water_uv = bmap_to_normals(bmap_for_entity(my,0), 50);			// *** use 1st skin texture, okay
   	}
